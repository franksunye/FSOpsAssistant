...
### 🔧 重要修复
- **升级通知重复发送**: 修复同一组织收到多条升级通知的问题
- **通知类型系统重构**: 使用REMINDER和ESCALATION替代VIOLATION和STANDARD，避免类型别名混乱
- **组织级升级任务**: 确保每个组织只创建一个升级任务，而不是为每个工单创建独立任务
- **消息显示配置**: 新增可配置的消息显示数量限制，支持Web界面动态调整

### ✨ 新增功能
- **消息显示配置**: 支持通过Web界面配置各类通知的最大显示工单数量
- **增强去重机制**: 完善升级任务去重检查，防止新旧任务并存
- **向后兼容**: 保持对旧通知类型的兼容，平滑过渡

### 📝 技术改进
- **升级任务聚合**: 升级通知按组织聚合，使用`ESCALATION_组织名`格式
- **通知发送优化**: 正确处理组织级别的升级通知，避免重复发送
- **配置管理增强**: 新增4个消息显示配置项，支持动态调整

## [0.3.0] - 2025-06-29

### 🔄 重大重构
- **统一数据模型**: 完成TaskInfo到OpportunityInfo的全面迁移
- **两级SLA机制**: 实现4小时提醒 + 8小时升级的分级通知
- **管理器架构**: DataStrategy + Notification + Decision分层管理
- **LLM决策优化**: 完整的DeepSeek AI决策增强支持

### ✨ 新增功能
- **两级SLA通知**: 提醒通知到组织群，升级通知到运营群
- **动态消息模板**: 基于数据库配置的智能消息格式化
- **智能缓存系统**: 多级缓存提升性能和数据一致性
- **统一数据架构**: OpportunityInfo模型的一致性设计
- **兼容性支持**: TaskInfo兼容性存根，平滑迁移

### 🐛 问题修复
- **重复工单消息**: 修复群消息中同一工单重复显示的问题
- **硬编码SLA阈值**: 修复消息模板中硬编码12小时的问题
- **通知统计数据**: 修复通知管理页面数据显示为0的问题
- **数据模型一致性**: 解决TaskInfo/OpportunityInfo混用问题

### 📚 文档更新
- **架构文档**: 更新系统架构图和组件描述
- **用户指南**: 更新核心功能和业务场景说明
- **开发文档**: 更新数据模型和API接口文档
- **测试文档**: 更新测试用例和fixture定义
