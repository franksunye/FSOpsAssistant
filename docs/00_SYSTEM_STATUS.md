...
FSOA (Field Service Operations Assistant) 是一个基于统一数据模型的企业级现场服务运营自动化解决方案，已完成TaskInfo到OpportunityInfo的全面迁移，实现两级SLA通知机制，系统架构成熟稳定。

## 🏗️ 架构状态

### ✅ v0.3.0 重大更新
- **统一数据模型**: 完成TaskInfo到OpportunityInfo的全面迁移
  - `DataStrategyManager`: 统一的数据获取和缓存管理
  - `NotificationManager`: 两级SLA通知任务管理
  - `DecisionEngine`: 规则+LLM的混合决策系统
- **两级SLA机制**: 4小时提醒 + 8小时升级的分级通知
- **动态消息模板**: 基于数据库配置的智能消息格式化
- **LLM决策优化**: 完整的DeepSeek AI决策增强支持

### 🆕 v0.3.0 核心功能
- **两级SLA通知**: 提醒(4h)到组织群，升级(8h)到运营群
- **动态SLA配置**: 支持不同状态商机的差异化阈值设置
- **智能消息路由**: 基于组织和通知类型的智能群组路由
- **统一数据架构**: OpportunityInfo模型的一致性设计
- **LLM决策增强**: 可选的AI决策优化和消息生成
- **完整兼容性**: TaskInfo兼容性存根，平滑迁移支持
