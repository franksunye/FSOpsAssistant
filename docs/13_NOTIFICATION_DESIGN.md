...
FSOA系统的通知模块是Agent工作流的执行终端，负责将SLA分析结果转化为具体的通知任务，并通过企微群智能发送给相关人员。该模块实现了完整的通知生命周期管理，包括任务创建、消息格式化、发送执行、状态追踪和重试机制。

### 1.1 设计目标

- **智能路由**：基于orgName的自动群组路由
- **分级通知**：违规、标准、升级的三级通知体系
- **可靠发送**：冷静期、重试机制确保通知可达
- **格式化消息**：标准模板+LLM优化的双重格式化
- **完整追踪**：从创建到发送的全生命周期记录
